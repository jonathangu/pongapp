...
import { isSoloCrossing, TOGETHER_RADIUS } from './solo'
...
export function advanceSeamanship(s: RescueState) {
  ...
  else if (lesson.step === 1 && (s.stats.rescues > 0 || target && Math.hypot(target.x - s.ship.x, target.y - s.ship.y) <= (isSoloCrossing(s) ? TOGETHER_RADIUS - .5 : 22))) lesson.step = 2
  else if (lesson.step === 2 && (isSoloCrossing(s) ? s.events.some(e => e.kind === 'together') : s.crew.some(p => !p.pet && ['east', 'west', 'north', 'south', 'starburst'].includes(p.seat ?? '')))) lesson.step = 3
  else if (lesson.step === 3 && s.stats.rescues > 0) lesson.step = 4
  else if (lesson.step === 4 && s.meal.remaining > 0 && (isSoloCrossing(s) || s.crew.some(p => !p.pet && p.seat === 'galley'))) {
