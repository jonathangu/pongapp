...
export function createRescueGame(options: { seed?: number; biome?: BiomeId; solo?: boolean; players?: Array<{ id: string; name: string }>; epoch?: number; inspiration?: string; region?: RescueRegion } = {}): RescueState {
  ...
  const state: RescueState = {
    ...
    crew, stations: RESCUE_STATIONS.map(s => ({ id: s.id, angle: s.angle, cooldown: 0, charge: 0, heat: 0, upgrade: null, operated: false, firing: false, lingering: 0 })),
...
export function restartRescueGame(s: RescueState, nextBiome = false): RescueState {
  const next = createRescueGame({ seed: nextBiome ? (s.initialSeed + 1777) >>> 0 : s.initialSeed, biome: nextBiome ? ((s.biome + 1) % 3) as BiomeId : s.biome,
    solo: s.solo, players: s.crew.filter(c => c.origin === 'human').map(c => ({ id: c.id, name: c.name })), epoch: s.epoch + 1, inspiration: s.inspiration, region: s.region })
