...
try {
  ...
  for(let world=0;world<5;world++){
    ...
    await evaluate(`(()=>{const s=qaProps.getState();s.tick=180+${world}*2160+260;s.crew.finishedTick=null;s.phase='playing';s.hearts=3;s.invulnerableTicks=900;s.boat.x=.5;s.boat.heading=0;s.distance=12;s.crew.choiceTicks=0;s.crew.shotCooldown=0;s.crew.bossSpawned=true;s.objects=[{id:8000,type:'predator',enemy:'ambusher',x:.7,y:.42,age:10,hp:18,maxHp:18,targetX:.5,targetY:.76},{id:8001,type:'predator',enemy:'chaser',x:.24,y:.61,age:10,hp:12,maxHp:12,targetX:.5,targetY:.76},{id:8002,type:'rescue',x:.66,y:.6},{id:8003,type:'gate',x:.35,y:.24},{id:8004,type:'relic',x:.54,y:.44},{id:8005,type:'firefly',x:.5,y:.56},{id:8006,type:'firefly',x:.4,y:.34},{id:8007,type:'rock',x:.21,y:.48}].map(o=>({radius:.04,phase:0,drift:0,...o}));s.nextObjectId=9000})()`)
    await evaluate("qaProps.onCrew({station:'shoot'})")
    ...
    await evaluate("qaProps.onCrew({station:'recover'})")
  ...
  await evaluate('qaProps.onCrew({station:"right"})');await sleep(1500)
