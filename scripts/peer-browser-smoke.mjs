...
try{
  ...
  const layout=await evaluate(mobile,"({overflow:document.documentElement.scrollWidth>innerWidth,canvas:document.querySelector('.expedition-canvas').getBoundingClientRect().toJSON(),control:document.querySelector('.river-paddle').getBoundingClientRect().toJSON()})")
