...
try{
  const result=await new Promise((resolve,reject)=>{
    ...
    socket.addEventListener('open',()=>socket.send(JSON.stringify({type:'hello',version:7,guestId:'stale-qa',displayName:'Old client',role:'player'})))
  ...
  console.log(JSON.stringify({server,oldProtocol:7,result,verifiedAt:new Date().toISOString()},null,2))
