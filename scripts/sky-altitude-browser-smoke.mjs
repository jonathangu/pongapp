...
for(const engine of (process.env.QA_ENGINES??'chromium,webkit').split(',')){
  ...
  try{
    ...
    for(const [width,height] of sizes){
      ...
      const b=await page.locator('[data-action=right]').boundingBox();await page.mouse.move(b.x+b.width/2,b.y+b.height/2);await page.mouse.down();await pause(1400);await page.mouse.up();await pause(400)
      ...
      await page.evaluate(()=>{const s=qaProps.getState();s.tick=1979;s.crew.altitudeEventIndex=0;s.crew.encounterIndex=0;s.boat.flight=null;s.boat.altitude=0;s.objects=[];s.rescued=0;s.crew.bossDefeated=false})
      ...
      await page.evaluate(()=>{const s=qaProps.getState();s.objects=[{id:91001,type:'predator',enemy:'ambusher',x:(s.boat.x+.12)%(Math.PI*2),y:.4,radius:.04,phase:0,drift:0,age:0,hp:1000,maxHp:1000,altitude:6,flight:{kind:'rise',tick:200,duration:600,peak:6}}];qaProps.onCrew({targetId:null,station:'shoot'})})
      ...
      await page.evaluate(()=>qaProps.onCrew({station:'recover'}))
      ...
      await page.evaluate(()=>{const s=qaProps.getState();s.tick=3599;s.crew.encounterIndex=0;s.crew.altitudeEventIndex=1;s.objects=[];s.crew.shots=[];s.crew.targetId=null;s.rescued=0;s.crew.bossDefeated=false;s.crew.bossesDefeated=0})
      await page.waitForFunction(()=>document.querySelector('.crew-boss-warning'))
      assert.match(await page.locator('.crew-boss-warning').textContent(),/CROWN OF THE DEEP APPROACHING/)
      ...
      await page.evaluate(()=>{const s=qaProps.getState(),boss=s.objects.find(o=>o.bossKind==='sentinel');s.objects=[boss];window.qaBossId=boss.id;qaProps.onCrew({targetId:null,station:'shoot'})})
      await page.waitForFunction(()=>qaProps.getState().crew.shots.some(s=>s.targetId===qaBossId))
      await page.waitForFunction(()=>qaProps.getState().crew.bossesDefeated===1,null,{timeout:45000})
      assert.equal(await page.evaluate(()=>qaProps.getState().crew.bossDefeated),false)
      await page.evaluate(()=>qaProps.onCrew({station:'recover'}))
      await page.evaluate(()=>{const s=qaProps.getState();s.tick=8639;s.crew.encounterIndex=1;s.crew.altitudeEventIndex=2;s.objects=[];s.crew.shots=[];s.rescued=0;s.crew.bossDefeated=false})
      await page.waitForFunction(()=>document.querySelector('.crew-boss-warning')?.textContent.includes('STORMWYRM SOVEREIGN'))
      ...
      await page.evaluate(()=>qaProps.onCrew({station:'shoot',targetId:null}))
      ...
      await snapshot('guardian-downward-aim');await page.evaluate(()=>qaProps.onCrew({station:'recover'}))
      await page.waitForFunction(()=>qaProps.getState().boat.altitude===0&&qaProps.getState().boat.flight===null)
