...
const rows=[]
for(const mode of ['idle','parked_repair','parked_shoot','coordinated'] as const)for(let seed=1;seed<=20;seed++){
  ...
  for(let t=0;t<11000&&s.phase!=='finished';t++){
    ...
    if(mode==='parked_repair'||mode==='parked_shoot')inputs={pilot:{paddle:0,station:mode==='parked_repair'?'recover':'shoot'},support:{paddle:0,station:'shoot'}}
...
console.log(JSON.stringify({runtimeSession:'01a0369d-0914-7190-ac0e-b4d37e1fc052',summary:['idle','parked_repair','parked_shoot','coordinated'].map(mode=>({mode,wins:rows.filter(r=>r.mode===mode&&r.win).length,runs:20})),rows},null,2))
