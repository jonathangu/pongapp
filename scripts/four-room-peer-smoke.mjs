...
async function verifyAltitude(a,b,path){
  ...
  await evaluate(a,"(()=>{const s=qa.peer.state;s.phase='playing';s.tick=1979;s.invulnerableTicks=10000;s.rescued=0;s.crew.bossDefeated=false;s.crew.altitudeEventIndex=0;s.crew.encounterIndex=0;s.boat.altitude=0;s.boat.flight=null;s.objects=[]})()")
  ...
  await evaluate(a,"(()=>{const s=qa.peer.state;s.objects=[{id:91001,type:'predator',enemy:'ambusher',x:s.boat.x,y:.2,radius:.04,phase:0,drift:0,age:0,hp:1000,maxHp:1000,attackPhase:'telegraph',attackTick:-10000,altitude:6,flight:{kind:'rise',tick:2000,duration:6000,peak:6}}]})()")
  await evaluate(b,"qa.setCrew({station:'shoot',targetId:null})")
  await waitFor(a,'qa.peer.state.crew.shots.some(s=>s.targetId===91001 && s.vAltitude>0)')
  try{await waitFor(b,'qa.peer.getState().crew.shots.some(s=>s.targetId===91001 && s.altitude>3)')}catch(error){console.log('Altitude relay diagnostic',path,await evaluate(a,'({tick:qa.peer.state.tick,players:qa.peer.state.players,shots:qa.peer.state.crew.shots,objects:qa.peer.state.objects,peer:qav.peer})'),await evaluate(b,'({tick:qa.peer.getState().tick,shots:qa.peer.getState().crew.shots,peer:qav.peer})'));throw error}
  await evaluate(b,"qa.setCrew({station:'recover'})")
  ...
  await evaluate(a,"(()=>{const s=qa.peer.state;s.tick=3599;s.crew.encounterIndex=0;s.crew.altitudeEventIndex=1;s.crew.bossesDefeated=0;s.objects=[]})()")
...
async function verifyHeldControls(a,b,path){
  ...
  await evaluate(b,"qa.setCrew({station:'shoot'})")
  await waitFor(a,'qa.peer.state.players['+JSON.stringify(id)+'].station==="shoot" && !qa.peer.state.players['+JSON.stringify(id)+'].deck.moving')
  await waitFor(b,'!qa.peer.getState().players['+JSON.stringify(id)+'].deck.moving')
  await sleep(800)
  const before=await evaluate(a,'qa.peer.state.crew.shotsFired')
  if(before<1)throw Error(path+' staffed cannon did not fire')
  await evaluate(a,'qa.peer.state.crew.repair=0')
  const stale=await evaluate(a,'({kind:"input",epoch:qa.peer.epoch,control:{...qa.peer.controls[qa.peer.remoteId]}})')
  await evaluate(b,"qa.setCrew({station:'recover'})")
  await waitFor(a,'qa.peer.state.players['+JSON.stringify(id)+'].deck.moving')
  const travelingShots=await evaluate(a,'qa.peer.state.crew.shotsFired')
  await evaluate(a,'qa.peer.receiveRelay('+JSON.stringify(JSON.stringify(stale))+')')
  await sleep(250)
  const transit=await evaluate(a,'({shots:qa.peer.state.crew.shotsFired,repair:qa.peer.state.crew.repair,station:qa.peer.state.players['+JSON.stringify(id)+'].station})')
  if(transit.shots!==travelingShots||transit.repair!==0||transit.station!=='recover')throw Error(path+' transit/stale invariant failed '+JSON.stringify(transit))
  await waitFor(a,'qa.peer.state.hearts>=2',10000);await waitFor(b,'qa.peer.getState().hearts>=2')
  await evaluate(b,"qa.setCrew({station:'right'})")
  await waitFor(a,'qa.peer.state.players['+JSON.stringify(id)+'].station==="right" && !qa.peer.state.players['+JSON.stringify(id)+'].deck.moving')
  const x=await evaluate(a,'qa.peer.state.boat.x');await sleep(900)
  const after=await evaluate(a,'qa.peer.state.boat.x')
  if(Math.abs(after-x)<.25)throw Error(path+' persistent helm did not steer')
  await evaluate(b,"qa.setCrew({station:'shoot',targetId:81001})")
  await waitFor(a,'qa.peer.state.crew.targetId===81001');await waitFor(b,'qa.peer.getState().crew.targetId===81001')
  await evaluate(b,'qa.setCrew({targetId:null})');await waitFor(a,'qa.peer.state.crew.targetId===null')
  await evaluate(a,'clearInterval(qaQuiet)')
  results.push({mode:'coop',path,stations:'travel delay, exclusive operation, persistent helm, repair and stale destination rejection passed',targetSelection:'passed'})
}
async function verifyTapBurst(a,b,path){
  const id=await evaluate(b,'qav.participant.id')
  await evaluate(b,"qa.setCrew({station:'left'})")
  await waitFor(a,'qa.peer.state.players['+JSON.stringify(id)+'].station==="left" && !qa.peer.state.players['+JSON.stringify(id)+'].deck.moving')
  ...
  await evaluate(b,"(()=>{const p=qa.peer,send=p.send.bind(p);p.send=()=>{};qa.setCrew({station:'right'});qa.setCrew({station:'shoot'});qa.setCrew({station:'recover'});p.send=send;p.sendControl();p.sendControl();p.sendControl()})()")
  await waitFor(a,'qa.peer.state.players['+JSON.stringify(id)+'].station==="recover"')
  await waitFor(a,'qa.peer.state.hearts===3',10000);await waitFor(b,'qa.peer.getState().hearts===3')
  for(const session of [a,b]){
    const actions=await evaluate(session,'qa.peer.getState().crew.actions['+JSON.stringify(id)+']')
    if(actions.recover!==(before.recover??0)+1||actions.right!==(before.right??0)||actions.shoot!==(before.shoot??0))throw Error(path+' coalesced destinations applied extra work '+JSON.stringify({before,actions}))
  }
  results.push({mode:'coop',path,coalescedDestinations:'latest destination only, duplicate packets once, no repair tap exploit'})
}
...
try{
  ...
  if(!process.env.QA_UI_ONLY) for(const mode of ['coop','versus']){
    ...
    const response=await evaluate(b,`new Promise(resolve=>{const p=qa.peer;const original=p.send.bind(p);p.send=()=>{};const before=${mode==='coop'?'p.getState().players[qav.participant.id].deck.x':'p.getState().racers[qav.participant.id].lane'};const at=performance.now();qa.${mode==='coop'?'setCrew({station:"right"})':'tap()'};const poll=()=>{const after=${mode==='coop'?'p.getState().players[qav.participant.id].deck.x':'p.getState().racers[qav.participant.id].lane'};if(after!==before){p.send=original;resolve({localResponseMs:performance.now()-at,before,after})}else if(performance.now()-at>300){p.send=original;resolve({localResponseMs:999,before,after})}else requestAnimationFrame(poll)};requestAnimationFrame(poll)})`)
  ...
  const multitouch={contract:'Four exclusive persistent rooms; physical travel verified by four-room-browser-smoke'}
  ...
  for(const mode of ['coop','versus']) {
    ...
    if(mode==='coop') {
      ...
      await evaluate(guest,"crewProps.onCrew({station:'recover'})")
      for(const session of [host,guest])await waitFor(session,"document.querySelector('.crew-hull').dataset.hearts==='3'",10000)
      ...
      results.push({health:'host+guest damage and guest physical-room repair passed',guestWidths:[320,375,390],vectorHearts:'visible'})
    } else {
