...
export class GameRoom extends DurableObject<Env> {
  ...
  private async ensureLoaded(): Promise<void> {
    ...
    if ((stored?.version === PROTOCOL_VERSION || stored?.version === 7 || stored?.version === 6 || stored?.version === 5) && stored.config) {
      ...
      this.game = stored.game && (stored.game.rulesetVersion === 9 || stored.game.rulesetVersion === 6) ? stored.game : null
