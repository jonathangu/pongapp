...
describe('local Two Oars room server', () => {
  it('creates an authoritative crew trip, acknowledges input, and persists v6', async () => {
    ...
    expect(await fetch(`${baseUrl}/api/health`).then((response) => response.json())).toMatchObject({ status: 'ok', protocol: 9 })
    ...
    expect(snapshot.type === 'snapshot' && snapshot.state.rulesetVersion).toBe(10)
    if (snapshot.type !== 'snapshot' || snapshot.state.rulesetVersion !== 10) throw new Error('Missing game')
