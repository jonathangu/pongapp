...
describe('local Two Oars room server', () => {
  it('creates an authoritative crew trip, acknowledges input, and persists v6', async () => {
    ...
    expect(await fetch(`${baseUrl}/api/health`).then((response) => response.json())).toMatchObject({ status: 'ok', protocol: 7 })
    ...
    expect(snapshot.type === 'snapshot' && snapshot.state.rulesetVersion).toBe(8)
    if (snapshot.type !== 'snapshot' || snapshot.state.rulesetVersion !== 8) throw new Error('Missing game')
