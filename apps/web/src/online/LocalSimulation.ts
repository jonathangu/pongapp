...
export interface CrewControl { steer: number; action: boolean; recoverHeld: boolean; station: CrewStation | null; upgrade: CrewUpgrade | null; targetId: number | null; tap?: CrewTap }
export interface Control extends CrewControl { paddle: number; taps: number; flares: number; seq: number; stationSeq: number; upgradeSeq: number; targetSeq: number; leftTaps: number; rightTaps: number; shootTaps: number; recoverTaps: number }
export type Controls = Record<string, Control>
export const neutralControl = (): Control => ({ paddle: 0, taps: 0, flares: 0, seq: 0, steer: 0, action: false, recoverHeld: false, station: null, upgrade: null, targetId: null, stationSeq: 0, upgradeSeq: 0, targetSeq: 0, leftTaps: 0, rightTaps: 0, shootTaps: 0, recoverTaps: 0 })
const TAP_COUNTERS = { left: 'leftTaps', right: 'rightTaps', shoot: 'shootTaps', recover: 'recoverTaps' } as const
const COUNTERS = ['taps','flares','stationSeq','upgradeSeq','targetSeq','leftTaps','rightTaps','shootTaps','recoverTaps'] as const
export function applyCrewControl(control: Control, patch: Partial<CrewControl>): void {
  ...
  if (patch.targetId !== undefined) control.targetSeq++
...
export function stepLocal(state: OnlineGameState, controls: Controls, consumed: Controls): void {
  ...
  for (const [id, control] of Object.entries(controls)) {
    ...
    inputs[id] = { paddle: state.rulesetVersion === 6 ? (tap ? 1 : 0) : control.paddle, flare, steer: control.steer, action: control.action, recoverHeld: control.recoverHeld,
      targetId: control.targetSeq > previous.targetSeq ? control.targetId : undefined,
