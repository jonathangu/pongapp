...
export class RoomClient {
  ...
  getRenderState(): CoopGameState {
    ...
    if (local?.rulesetVersion === 10) return local
  ...
  setCrew(patch: Partial<CrewControl>): void { this.peer?.setCrew(patch) }
  ...
  private onMessage(raw: string): void {
    ...
    if (message.type === 'welcome') {
      ...
      if (message.state.rulesetVersion !== 10) return
      if (this.peer) return
      ...
      if (me && me.slot !== null && other) {
        this.peer = new PeerSession(me.id, other.id, me.slot === 0, message.state,
          ...
          (state) => {
            if (state.rulesetVersion !== 10) return
