...
export function OnlineVersusRoom({ serverUrl, roomCode, createRequest, identity, onExit }: {
  ...
  return <VersusRiver initialState={view.gameState} subscribe={subscribe} localPlayerId={view.participant.id} title={title} roomCode={view.roomCode} latencyMs={view.latencyMs} reconnecting={view.status === 'connecting'} onTap={tap} onExit={exit} onRematch={rematch}/>
}
