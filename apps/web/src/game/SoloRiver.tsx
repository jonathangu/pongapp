...
function scoutPaddle(state: CoopGameState, humanPaddle: number): number {
  ...
  const danger = state.objects
    .filter((object) => (object.type === 'rock' || object.type === 'log' || object.type === 'predator') && object.y > 0.43 && object.y < 0.78)
    .sort((a, b) => b.y - a.y)[0]
  if (danger && Math.abs(danger.x - state.boat.x) < 0.16) return danger.x >= state.boat.x ? 1 : 0
  const prize = state.objects
    .filter((object) => object.type !== 'rock' && object.type !== 'log' && object.type !== 'predator' && object.y > 0.28 && object.y < 0.72)
    .sort((a, b) => b.y - a.y)[0]
...
export function SoloRiver({ playerName, onExit }: { playerName: string; onExit: () => void }) {
  ...
  useEffect(() => {
    const timer = window.setInterval(() => {
      ...
      advanceCoopGame(state, { [HUMAN_ID]: { paddle: paddleRef.current, flare: flareRef.current }, [SCOUT_ID]: { paddle: scoutPaddle(state, paddleRef.current) } })
  ...
  const flare = useCallback(() => { flareRef.current = true }, [])
  ...
  return <CoopRiver getState={getState} subscribe={subscribe} localPlayerId={HUMAN_ID} title="Solo Adventure" roomCode="SCOUT AI" network={{ latencyMs: null, quality: 'good', reconnecting: false }} onPaddle={paddle} onFlare={flare} onExit={onExit} onRematch={rematch} modeLabel="Scout is reading the river" />
}
