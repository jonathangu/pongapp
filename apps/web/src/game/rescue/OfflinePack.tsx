...
export function OfflinePack({ required = false, onReady }: { required?: boolean; onReady?: (ready: boolean) => void }) {
  ...
  const [installable, setInstallable] = useState(false), [installed, setInstalled] = useState(isStandalone)
  ...
  const download = async () => {
    ...
    channel.port1.onmessage = event => {
      ...
      else if (v.type === 'complete') { setStatus(v); setBusy(false); setProgress(1); setMessage(standalone ? 'Your installed game is ready. Set sail, online or offline.' : 'Ready for offline solo play. You can install the game now.') }
  ...
  const checking = status.ready === undefined || !manifest
  if (required && !standalone) return <section className="sr-offline-section install-first" aria-label="Required phone installation"><p className="sr-kicker">ONE-TIME SETUP · 1 OF 2</p><h2>A little world.<br/>On your home screen.</h2><p>Install Starling, then open its icon to download the complete game and all seven songs. Everyone installs on their own phone.</p><button className="g-primary" onClick={() => void install()}>{installed ? 'Open Starling from your home screen' : installable ? 'Install Starling →' : 'Install on your phone →'}</button><div className="sr-install-help"><b>iPhone / iPad</b><p>Open this page in Safari → Share → Add to Home Screen → Add. Keep “Open as Web App” on if shown. Then tap the Starling icon.</p><b>Android</b><p>Open in Chrome → menu → Install app / Add to home screen. Then open Starling.</p><b>Computer</b><p>Use Chrome or Edge’s install icon or “Install this page as an app” menu.</p></div><p className="sr-small">The browser cannot open your home-screen icon for you. After opening it, download the offline pack there. No account or app-store purchase.</p><p className="sr-small">Offline: solo with your crew. Online: play together, one phone each.</p>{help && <p role="status">Follow your device’s steps above, then launch the Starling icon.</p>}</section>
  return <section className="sr-offline-section"><div><p className="sr-kicker">TAKE THE WHOLE LITTLE WORLD WITH YOU</p><h2>{standalone ? 'Your ship is installed.' : 'Make room on your home screen.'}</h2><p>{standalone ? 'Download the complete game and all seven songs here. Then sail solo offline or play together online.' : 'Install Starling from your browser, open its icon, then download the complete game there. No app-store account needed.'}</p><p className="sr-small">Co-op needs internet. Saves stay on your device; export one before switching phones. Browser storage can be evicted, so check your pack before traveling.</p></div><div className="sr-pack-actions"><button className="sr-primary" disabled={busy || checking || Boolean(status.ready && !update)} onClick={() => void download()}>{checking ? 'Checking game pack…' : busy ? `Downloading · ${Math.round(progress * 100)}%` : update ? 'Download updated pack' : status.ready ? '✓ Offline pack ready' : `Download offline pack${manifest ? ` · ${(manifest.bytes / 1048576).toFixed(1)} MB` : ''}`}</button>{busy && <><progress value={progress} max="1" aria-label="Offline pack download progress"/><button onClick={() => worker.current?.postMessage({ type: 'starling-cancel' })}>Cancel download</button></>}<button onClick={() => void install()} disabled={installed}>{installed ? '✓ Installed' : installable ? 'Install Starling' : 'Install on your phone'}</button><p role="status">{message}</p>{help && <div className="sr-install-help"><b>iPhone / iPad</b><p>Open in Safari, tap Share, then Add to Home Screen. If offered, enable Open as Web App.</p><b>Android</b><p>Open in Chrome, tap its menu, then Install app or Add to home screen.</p><b>Desktop</b><p>Use your browser’s install icon or app menu. Firefox can still download the offline pack without a standalone install.</p><p>Open the installed icon, then download the pack there. This is a web app, not an APK or IPA.</p><button onClick={() => setHelp(false)}>Got it</button></div>}</div></section>
}
