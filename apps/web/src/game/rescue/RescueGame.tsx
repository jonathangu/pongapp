...
import { availableRescueCrew, FOG_COLUMNS, FOG_ROWS, nearRescueStation, nearestRescueDock, RESCUE_ABILITIES, RESCUE_BIOMES, RESCUE_BUTTON, RESCUE_STATIONS, rescueEnemyRecipe, rescueUpgradeCost, stationSpec, type RescueState, type ShipUpgrade, type StationId } from '@pongapp/game-core'
...
export function RescueGame({ options, onExit }: { options: RescueSessionOptions; onExit: () => void }) {
  ...
  useEffect(() => {
    ...
    let raf = 0, last = performance.now(), accumulator = 0, lastUi = 0, eventId = 0, eventEpoch = 0, previousSeat: StationId | null = null
    const commandKey = (event: KeyboardEvent) => {
      if (event.code !== 'KeyC' || event.repeat || event.target instanceof HTMLInputElement || event.target instanceof HTMLTextAreaElement) return
    ...
    const frame = (now: number) => {
      ...
      const seat = session.state.crew.find(c => c.id === session.playerId)?.seat ?? null
      if (seat === 'map' && previousSeat !== 'map') setModal('map')
      previousSeat = seat
      ...
      for (const e of session.state.events) if (e.id > eventId) {
        ...
        if (e.kind === 'meal') setToast('Dinner’s ready · crew speed +20%')
      }
    ...
    return () => { cancelAnimationFrame(raf); resize.disconnect(); container.removeEventListener('pointerdown', unlock); window.removeEventListener('keydown', unlock); window.removeEventListener('keydown', commandKey); document.removeEventListener('visibilitychange', visibility); session.dispose(); controls.dispose(); audio.dispose(); scene.dispose(); runtime.current = null; Reflect.deleteProperty(window, '__STARLING__') }
  }, [options])
  ...
  const hold = (bit: number) => ({ onPointerDown: (e: PointerEvent<HTMLButtonElement>) => { e.preventDefault(); e.currentTarget.setPointerCapture(e.pointerId); if (runtime.current) runtime.current.controls.press(bit) }, onPointerUp: () => { if (runtime.current) runtime.current.controls.buttons &= ~bit }, onPointerCancel: () => { if (runtime.current) runtime.current.controls.buttons &= ~bit }, onLostPointerCapture: () => { if (runtime.current) runtime.current.controls.buttons &= ~bit } })
  ...
  return <div className={`sr-game ${settings.reducedMotion ? 'sr-reduced' : ''}`} ref={shell}>
    ...
    {state && <>
      ...
      <div className="sr-threats" aria-label="Nearby enemy bearings">{[...state.enemies].sort((a, b) => (a.phase === 'tell' ? -100 : 0) - (b.phase === 'tell' ? -100 : 0) + Math.hypot(a.x - state.ship.x, a.y - state.ship.y) - Math.hypot(b.x - state.ship.x, b.y - state.ship.y)).slice(0, 4).map(enemy => <span key={enemy.id} className={enemy.phase === 'tell' ? 'telling' : ''}><i style={{ transform: `rotate(${Math.PI / 2 - Math.atan2(enemy.y - state.ship.y, enemy.x - state.ship.x)}rad)` }}>↑</i>{rescueEnemyRecipe(state, enemy.kind)?.name ?? (enemy.kind === 'guardian' ? 'Guardian' : enemy.kind === 'needle' ? 'Needlefish' : enemy.kind)}{enemy.phase === 'tell' && <b> !</b>}</span>)}</div>
      ...
      <div className="sr-keyboard-hint">WASD / arrows · Space jump / exit · E enter · F action · Q throw · C crew</div>
