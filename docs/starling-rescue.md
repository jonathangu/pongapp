...
## Public acceptance follow-up

Initial release PR22 / `22cba6735d953acc34522e3c01b314393adc9002` passed build/deploy, public asset integrity, eight-client rooms, full delayed/lossy authoritative voyage, and public offline install/save in allthree viewports. A final real-browser reconnect check exposed a silent closing transport: the host's authoritative tick stopped at120 while its status remainedOnline, and the other peer advanced to2526 with AI takeover. Browser close callbacks alone are insufficient. The release follow-up explicitly handles offline/online events, replaces silent transports aftereightseconds, rejects stale callbacks, preserves terminal seat-transfer policy, and removes listeners/timers on exit. Regression evidence is linked in the release ledger; final public acceptance is required before completion.
